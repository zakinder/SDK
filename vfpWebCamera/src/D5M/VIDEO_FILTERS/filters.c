...
void CgCoef(Coef *pCf) {
    d5m_write_reg(D5M_BASE, w_kernal_1_reg_08, pCf->K1);
    d5m_write_reg(D5M_BASE, w_kernal_2_reg_09, pCf->K2);
    d5m_write_reg(D5M_BASE, w_kernal_3_reg_10, pCf->K3);
    d5m_write_reg(D5M_BASE, w_kernal_4_reg_11, pCf->K4);
    d5m_write_reg(D5M_BASE, w_kernal_5_reg_12, pCf->K5);
    d5m_write_reg(D5M_BASE, w_kernal_6_reg_13, pCf->K6);
    d5m_write_reg(D5M_BASE, w_kernal_7_reg_14, pCf->K7);
    d5m_write_reg(D5M_BASE, w_kernal_8_reg_15, pCf->K8);
    d5m_write_reg(D5M_BASE, w_kernal_9_reg_16, pCf->K9);
    d5m_write_reg(D5M_BASE, w_kSet_reg_17, pCf->Kc);
    usleep(1);
    video_feature_select(pCf->Vid);
}
void keyCgainConfigSelect() {
    ...
    while (menu_Active == TRUE) {
        ...
        case KEYPRESS_ARROW_DOWN:
            if (kValues <= 1) kValues = 0;
            else kValues -= pKey.ValueDown;
            keySelect = menu_select;
            break;
        case KEYPRESS_ARROW_UP:
            if (kValues > pKey.ValueMax) kValues = pKey.ValueMax + 1;
            else kValues += pKey.ValueUp;
            keySelect = menu_select;
            break;
        case KEYPRESS_ARROW_RIGHT:
            if (kValues > pKey.ValueMax) kValues = pKey.ValueMax + 1;
            else kValues += pKey.ValueRight;
            keySelect = menu_select;
            break;
        case KEYPRESS_ARROW_LEFT:
            if (kValues <= 1) kValues = 1;
            else kValues -= pKey.ValueLeft;
            keySelect = menu_select;
            break;
        case kernalconfig:
            kernel_config(&pCf);
