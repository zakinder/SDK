...
void HARDWARE_ETHERNETIO_CONTROL(char *p, int len) {
    ...
    d5m_write_reg(D5M_BASE, w_threshold_reg_4, value);
    if (value == 0) {
        camera_hdmi_config();
    }
    if (value == 1) {
...
//    xil_printf("02.Set to YCbCr 4:2:2 Mode\n\r");
        ...
        // 3 - Sharp Video
        // 4 - normal Video
        video_feature_select(0x0004);
        config_d5m_gen_testpattern(0x0004);
        xil_printf("normal Video\n\r");
    }
    if (value == 3) {
        video_feature_select(0x0003);
...
//    xil_printf("04.exposer-50 %d\n\r", exposer);
    ...
    if (value == 5) {
        g1_gain++;
        d5m_i2c_write(43, g1_gain);    //Green1 Gain
        xil_printf("05.Updated g1_gain+1 %d\n\r", g1_gain);
    }
    if (value == 6) {
        b_gain++;
        d5m_i2c_write(44, b_gain);    //Blue Gain
        xil_printf("06.Updated b_gain+1 %d\n\r", b_gain);
    }
    if (value == 7) {
        r_gain = r_gain + 0x1;
        d5m_i2c_write(45, r_gain);    //Red Gain
        xil_printf("07.Updated r_gain+1 %d\n\r", r_gain);
    }
    if (value == 8) {
        g2_gain = g2_gain + 0x1;
        d5m_i2c_write(46, g2_gain);    //Green2 Gain
...
int DO_404(struct tcp_pcb *pcb, char *req, int rlen) {
    ...
    if (tcp_sndbuf(pcb) < hlen) {
        xil_printf(
                "cannot send 404 message, tcp_sndbuf = %d bytes, message length = %d bytes\r\n",
                tcp_sndbuf(pcb), hlen);
        return -1;
    }
    ...
    while (fsize > 0) {
        ...
        if ((err = tcp_write(pcb, buf, n, 3)) != ERR_OK) {
            xil_printf(
                    ...
                    filename, fsize - n);
            xil_printf(
                    "attempted to lwip_write %d bytes, tcp write error = %d\r\n",
                    n, err);
            break;
        }
        tcp_output(pcb);
