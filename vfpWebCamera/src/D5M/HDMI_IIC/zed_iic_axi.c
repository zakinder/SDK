...
int zed_iic_axi_IicRead(zed_iic_t *pIIC, Xuint8 ChipAddress, Xuint8 RegAddress,
        Xuint8 *pBuffer, Xuint8 ByteCount);
int zed_iic_axi_IicEWrite(zed_iic_t *pIIC, Xuint8 ChipAddress,
        Xuint16 RegAddress, Xuint8 *pBuffer, Xuint8 ByteCount);
int zed_iic_axi_IicERead(zed_iic_t *pIIC, Xuint8 ChipAddress,
        Xuint16 RegAddress, Xuint8 *pBuffer, Xuint8 ByteCount);
/******************************************************************************
...
int zed_iic_axi_init(zed_iic_t *pIIC, char szName[], Xuint32 CoreAddress) {
    ...
    if (sizeof(zed_iic_axi_t) > ZED_IIC_CONTEXT_BUFFER_SIZE) {
        xil_printf(
                "ZED_IIC_CONTEXT_BUFFER_SIZE is not large enough for fic_iic_xps_t structure (increase to %d)\n\r",
                sizeof(zed_iic_axi_t));
        return 0;
    }
...
int zed_iic_axi_IicWrite(zed_iic_t *pIIC, Xuint8 ChipAddress, Xuint8 RegAddress,
    ...
    SentByteCount = XIic_DynSend(pContext->CoreAddress, ChipAddress,
            WriteBuffer, ByteCount + 1, XIIC_STOP);
    if (SentByteCount < 1) {
        SentByteCount = 1;
    }
...
int zed_iic_axi_IicRead(zed_iic_t *pIIC, Xuint8 ChipAddress, Xuint8 RegAddress,
    ...
    do {
        ...
        if (!(StatusReg & XIIC_SR_BUS_BUSY_MASK)) {
            SentByteCount = XIic_DynSend(pContext->CoreAddress, ChipAddress,
                    (Xuint8 *) &RegAddress, 1, XIIC_REPEATED_START);
        }
        cnt++;
    ...
    // Receive the data.
    ReceivedByteCount = XIic_DynRecv(pContext->CoreAddress, ChipAddress,
            pBuffer, ByteCount);
