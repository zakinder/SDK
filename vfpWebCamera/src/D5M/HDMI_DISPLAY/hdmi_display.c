...
void color_bars_vdma_hdmi(hdmi_display_start *pvideo) {
    int ret;
    Xuint32 iterations = 0;
    xil_printf("\n\r");
    xil_printf("HDMI IIC Initialization ...\n\r");
    ret = zed_iic_axi_init(&(pvideo->hdmi_out_iic), "ZED HDMI I2C Controller",
            pvideo->uBaseAddr_IIC_HdmiOut);
    if (!ret) {
    ...
    xil_printf("HDMI Output Initialization ...\n\r");
    {
        ...
        for (i = 0; i < CARRIER_HDMI_OUT_CONFIG_LEN; i++) {
            pvideo->hdmi_out_iic.fpIicWrite(&(pvideo->hdmi_out_iic),
                    carrier_hdmi_out_configs[i][0],
                    carrier_hdmi_out_configs[i][1],
                    &(carrier_hdmi_out_configs[i][2]), 1);
        }
    }
    xil_printf("Clear Frame Buffer\n\r");
    clear_color_bars();
    xil_printf("Video DMA (Output Side) Initialization ...\n\r");
    vgen_init(&(pvideo->vtc_hdmio_generator),
...
void bars(hdmi_display_start *pvideo) {
    ...
    color_bars(0);
    u32 menu_enable = 0;
    while (d5m_read_reg(D5M_BASE,r_sec_reg_60) != 5) {
        if (iterations > 0) {
            xil_printf("\n\rPress ENTER to re-start ...\n\r");
        }
        iterations++;
...
void d5m_vdma_hdmi(hdmi_display_start *pvideo) {
    int ret;
    int i;
    ret = zed_iic_axi_init(&(pvideo->hdmi_out_iic), "HDMI I2C Controller",
            pvideo->uBaseAddr_IIC_HdmiOut);
    if (ret != XST_SUCCESS) {
    ...
    for (i = 0; i < CARRIER_HDMI_OUT_CONFIG_LEN; i++) {
        pvideo->hdmi_out_iic.fpIicWrite(&(pvideo->hdmi_out_iic),
                carrier_hdmi_out_configs[i][0], carrier_hdmi_out_configs[i][1],
                &(carrier_hdmi_out_configs[i][2]), 1);
    }
    vfb_common_init(pvideo->uDeviceId_VDMA_HdmiDisplay, &(pvideo->vdma_hdmi));
    vfb_tx_init(&(pvideo->vdma_hdmi), &(pvideo->vdmacfg_hdmi_read),
            pvideo->hdmio_resolution, pvideo->hdmio_resolution,
            pvideo->video_address, pvideo->uNumFrames_HdmiDisplay);
    vgen_init(&(pvideo->vtc_hdmio_generator),
            pvideo->uDeviceId_VTC_HdmioGenerator);
    vgen_config(&(pvideo->vtc_hdmio_generator), pvideo->hdmio_resolution, 2);
}
void manual_vdma_config(hdmi_display_start *pvideo) {
    int ret;
    int i;
    ret = zed_iic_axi_init(&(pvideo->hdmi_out_iic), "HDMI I2C Controller",
            pvideo->uBaseAddr_IIC_HdmiOut);
