...
int CMD_PL_PS_STREAMER_LED_ON(char *buf) {
    return (!strncmp(buf, "CMD", 3)
            && !strncmp(buf + 4, "PL_PS_STREAMER_LED_ON", 21));
}
int CMD_PL_PS_STREAMER_LED_PAT(char *buf) {
    return (!strncmp(buf, "CMD", 3)
            && !strncmp(buf + 4, "PL_PS_STREAMER_LED_PAT", 22));
}
int CMD_PL_PS_STREAMER_LED_OFF(char *buf) {
    return (!strncmp(buf, "CMD", 3)
            && !strncmp(buf + 4, "PL_PS_STREAMER_LED_OFF", 22));
}
...
int CMD_PL_PS_STREAMER_ASSERT(char *buf) {
    return (!strncmp(buf, "CMD", 3)
            && !strncmp(buf + 4, "PL_PS_STREAMER_ASSERT", 21));
}
int CMD_PL_PS_STREAMER_CORE(char *buf) {
    return (!strncmp(buf, "CMD", 3)
            && !strncmp(buf + 4, "PL_PS_STREAMER_CORE", 19));
}
void extract_file_name(char *filename, char *req, int rlen, int maxlen) {
    ...
    if (*fend == '/') strcat(filename, "index.html");
}
...
int GENERATE_HTTP_HEADER(char *buf, char *fext, int fsize) {
    ...
    if (fext == NULL) strcat(buf, "text/html");
    else if (!strncmp(fext, "htm", 3)) strcat(buf, "text/html");
    else if (!strncmp(fext, "jpg", 3)) strcat(buf, "image/jpeg");
    else if (!strncmp(fext, "gif", 3)) strcat(buf, "image/gif");
    else if (!strncmp(fext, "json", 8)) strcat(buf, "text/json");
    else if (!strncmp(fext, "js", fsize)) strcat(buf, "application/javascript");
    else if (!strncmp(fext, "pdf", 2)) strcat(buf, "application/pdf");
    else if (!strncmp(fext, "css", 2)) strcat(buf, "text/css");
    else strcat(buf, "text/plain");
    strcat(buf, "\r\n");
...
http_arg *palloc_arg() {
    ...
    httpArgArrayIndex++;
    if (httpArgArrayIndex == 1000) httpArgArrayIndex = 0;
    a->count = p_arg_count++;
    a->fd = -1;
    a->fsize = 0;
    return a;
